...
class CkMime  : public CkMultiByteBase
{
	...
	// If the MIME is PKCS7 encrypted, this method can decrypt. Information about the
	// certificates required for decryption is always embedded within a PKCS7 enveloped
	// (i.e. encrypted) message. This method will automatically use this information to
	// locate the certificate + private key in Windows-based registry stores. If the
	// cert + private key was previously installed, such as from a PFX, then the
	// Decrypt method should be able to find it.
	// 
	// If the required cert + private key was NOT previously installed, then it is
	// possible to use PFX files directly. See the AddPfxSourceData and
	// AddPfxSourceFile methods for more information. (Also, see the example linked
	// below.)
	// 
	bool Decrypt(void);
