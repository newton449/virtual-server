...
class CkImap  : public CkMultiByteBase
{
	...
	// Polls the connection to see if any real-time updates are available. This method
	// does not send a command to the IMAP server, it simply checks the connection for
	// already-arrived messages that the IMAP server sent. This method would only be
	// called after IDLE has already been started via the IdleStart method.
	// 
	// If updates are available, they are returned in an XML string having this format:
	// 
	// If no updates have been received, the returned XML string looks like this:
	// 
	// On error, the method returns _NULL_
	// 
