...
: pSocket(NULL), pRequest(NULL), pResponse(NULL), pInput(NULL), pOutput(NULL),
queue(queue), mapping(mapping), id(id){
}
...
void RequestHandlerThread::run(){
	...
	pSocket = queue.deQ();
	while (pSocket != NULL){
		LOG(DEBUG) << "Got a socket from " << pSocket->System().getRemoteIP(pSocket);
...
: mapping(mapping), pListener(NULL), threadCount(threadCount),
port(port){
}
